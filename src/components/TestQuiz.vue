<template>
    <div class="quiz-page">
      <!-- Przycisk powrotu do menu głównego w lewym górnym rogu -->
      <button @click="goToMainMenu" class="main-menu-btn"> </button>
      ...
      <div v-else>
        ...
        <div class="answers-section">
          <ul>
            <li
              v-for="(answer, index) in displayedQuestion.answers"
              :key="index"
              class="answer-item"
            >
              <button
              ...
              >
                {{ answer.text }}
                <!-- Ikona ptaszka, widoczna gdy odpowiedź jest zaznaczona -->
        ...
        <div v-if="showAnswerForDisplayed" class="feedback">
          <p v-if="displayedQuestionCorrect" class="feedback-correct">
            Poprawna odpowiedź!
          </p>
          ...
          <!-- Nawigacja -->
          <div v-if="!isReview" class="navigation-btns">
            <!-- Przycisk poprzedniego pytania jako mała kwadratowa ikona -->
            <button
              v-if="currentQuestionIndex > 0 && answersHistory[currentQuestionIndex - 1]"
          ...
          <!-- W trybie przeglądu pytania -->
          <div v-else>
            <button @click="exitReview" class="next-btn">
              Powrót do bieżącego pytania
            </button>
          </div>
        </div>
      </div>
  ...
  export default {
    ...
    setup() {
      ...
      // Pobieramy folder z query (z poprzedniej strony)
      ...
      // Historia odpowiedzi – dla każdego pytania zapisujemy { selected, correct }
      const answersHistory = ref([]);
      // Aktualnie wybrane odpowiedzi dla bieżącego pytania
      const selectedAnswers = ref([]);
      // Flaga potwierdzenia wyboru dla bieżącego pytania
      const showAnswer = ref(false);
      const score = ref(0);
      // Jeśli w trybie przeglądu, przechowujemy indeks przeglądanego pytania
      ...
      const displayedQuestion = computed(() => {
        return isReview.value ? questions.value[reviewIndex.value] : currentQuestion.value;
      });
  
      // Dla wyświetlanego pytania – flaga określająca, czy pokazać odpowiedzi
      const showAnswerForDisplayed = computed(() => {
        return isReview.value || showAnswer.value;
      });
  
      // Dla wyświetlanego pytania – które odpowiedzi zostały wybrane?
      const displayedSelectedAnswers = computed(() => {
        if (isReview.value) {
          return answersHistory.value[reviewIndex.value]
            ? answersHistory.value[reviewIndex.value].selected
            : [];
        } else {
      ...
      // Dla wyświetlanego pytania – czy odpowiedź była poprawna?
      const displayedQuestionCorrect = computed(() => {
        if (isReview.value) {
          return answersHistory.value[reviewIndex.value]
            ? answersHistory.value[reviewIndex.value].correct
            : false;
        } else {
        ...
        try {
          ...
          if (result.success) {
            ...
            if (loadedQuestions.length === 0) {
              ...
              const testNameResult = await window.electronAPI.readFile({
                folder,
      ...
      // Przetwarzanie pliku na obiekt pytania
      ...
      // Funkcja tasująca elementy tablicy (Fisher-Yates)
      ...
      // Zaznaczanie/odznaczanie odpowiedzi (tylko w trybie odpowiadania)
      ...
      // Pomocnicza funkcja – czy dana odpowiedź jest zaznaczona?
      const isSelected = (answer) => {
        return displayedSelectedAnswers.value.includes(answer);
      };
  
      // Potwierdzenie wyboru dla bieżącego pytania
      ...
      // Przejście do poprzedniego pytania (tryb przeglądu)
      const reviewPrevious = () => {
        if (
          currentQuestionIndex.value > 0 &&
          answersHistory.value[currentQuestionIndex.value - 1]
        ) {
          reviewIndex.value = currentQuestionIndex.value - 1;
        }
      };
  
      // Powrót z trybu przeglądu do bieżącego pytania
      const exitReview = () => {
        reviewIndex.value = null;
      };
  
      // Przycisk powrotu do menu głównego – przekierowanie na "/"
      const goToMainMenu = () => {
        router.push("/");
      };
  ...
  /* Przycisk powrotu do menu głównego – mały, kwadratowy, umieszczony w lewym górnym rogu */
  .main-menu-btn {
    position: absolute;
    top: 10px;
    left: 10px;
  ...
  /* Stylowanie przycisków odpowiedzi */
  .answer-btn {
    position: relative;
  ...
  /* Ikona ptaszka w prawym górnym rogu – biały kolor */
  .checkmark {
    position: absolute;
    top: 4px;
    right: 4px;
  ...
  /* Styl dla poprawnych odpowiedzi (zielone tło) */
  .answer-btn.correct {
    background: #42b983;
    color: #fff;
  }
    
  /* Styl dla niepoprawnych odpowiedzi (czerwone tło) */
  .answer-btn.incorrect {
    background: #ff4444;
    color: #fff;
  }
    
  /* Styl dla poprawnych odpowiedzi, które nie zostały zaznaczone przez użytkownika (żółte tło) */
  .answer-btn.missed {
    background: #f1c40f;
    color: #fff;
  }
  ...
  /* Przycisk poprzedniego pytania – mały, kwadratowy, bez tekstu (tylko ikona) */
  .prev-btn {
    width: 40px;
    height: 40px;
    padding: 0;
